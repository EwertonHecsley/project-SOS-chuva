...
import { Plus, Heart, MapPin, Phone, Clock, User } from 'lucide-react';
...
export default function NeedHelpDashboard() {
  const { user } = useAuth();
  const [needs, setNeeds] = useState<Need[]>(mockNeeds.filter(n => n.userId === user?.id));
  ...
  const handleSubmitNeed = (e: React.FormEvent) => {
    e.preventDefault();
  ...
  const handleRequestHelp = (volunteerId: string, volunteerName: string) => {
    alert(`Solicitação de ajuda enviada para ${volunteerName}! O voluntário será notificado.`);
  };
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid md:grid-cols-2 gap-4 mb-8">
          ...
          >
            ...
            <h3 className="font-semibold text-lg mb-1">Cadastrar Necessidade</h3>
            <p className="text-sm text-gray-600">Informe o que você precisa e o nível de urgência</p>
          </button>
          ...
          >
            ...
            <p className="text-sm text-gray-600">Veja quem está disponível para ajudar</p>
          </button>
        </div>

        {/* My Needs */}
        <div className="mb-8">
          <h2 className="text-xl font-bold mb-4">Minhas Necessidades ({needs.length})</h2>
          {needs.length === 0 ? (
                    ...
                    <div>
                      <div className="flex items-center gap-2 mb-2">
                        <span className={`px-3 py-1 rounded-full text-xs font-bold ${
                          ...
                          {need.urgency === 'critical' ? 'CRÍTICO' :
                           need.urgency === 'high' ? 'ALTO' :
                           need.urgency === 'medium' ? 'MÉDIO' : 'BAIXO'}
                        </span>
                        ...
                        <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                          ...
                          {need.status === 'pending' ? 'Aguardando' :
                           need.status === 'in-progress' ? 'Em Andamento' : 'Resolvido'}
                        </span>
                      </div>
                      <h3 className="font-semibold mb-2">{need.description}</h3>
                      <div className="flex items-center gap-2 text-sm text-gray-600">
                        <Clock className="w-4 h-4" />
                        Cadastrado em {new Date(need.createdAt).toLocaleString('pt-BR')}
                      </div>
                    </div>
                  </div>
                  {need.volunteerId && (
                    <div className="mt-3 p-3 bg-blue-50 rounded-lg">
                      <p className="text-sm text-blue-800">
                        ✓ Um voluntário se ofereceu para ajudar com esta necessidade!
                      </p>
                    </div>
                  )}
                </div>
      ...
      {showNeedForm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg max-w-lg w-full p-6 max-h-[90vh] overflow-y-auto">
            <h3 className="text-xl font-bold mb-4">Cadastrar Nova Necessidade</h3>
            <form onSubmit={handleSubmitNeed} className="space-y-4">
              ...
              <div>
                ...
                >
                  ...
                  <option value="critical">Crítico - Emergência imediata</option>
                </select>
              </div>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              {volunteers.filter(v => v.status === 'available').map(volunteer => (
                <div key={volunteer.id} className="border border-gray-200 rounded-lg p-4">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-600">{volunteer.helpedCount} pessoas ajudadas</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <button
                    onClick={() => handleRequestHelp(volunteer.id, volunteer.name)}
