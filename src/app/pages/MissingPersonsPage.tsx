...
import { Search, MapPin, Clock, User, Baby, Dog, Plus, Phone } from 'lucide-react';
...
export default function MissingPersonsPage() {
  const { user } = useAuth();
  const [missingPersons, setMissingPersons] = useState<MissingPerson[]>(mockMissingPersons);
      ...
      <div className="bg-orange-600 text-white py-6 shadow-md">
        <div className="container mx-auto px-4">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-2xl font-bold">Pessoas e Animais Desaparecidos</h1>
          </div>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid md:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-white p-4 rounded-lg shadow">
            <div className="text-2xl font-bold text-blue-600">
              {missingPersons.filter(mp => mp.type === 'person' && mp.status === 'missing').length}
            </div>
            <div className="text-sm text-gray-600">Pessoas</div>
          </div>
          <div className="bg-white p-4 rounded-lg shadow">
            <div className="text-2xl font-bold text-pink-600">
              {missingPersons.filter(mp => mp.type === 'child' && mp.status === 'missing').length}
            </div>
            <div className="text-sm text-gray-600">Crianças</div>
          </div>
          <div className="bg-white p-4 rounded-lg shadow">
            <div className="text-2xl font-bold text-green-600">
              {missingPersons.filter(mp => mp.type === 'animal' && mp.status === 'missing').length}
            </div>
        ...
        <div className="bg-white rounded-lg shadow p-6 mb-8">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            <div>
              <h3 className="font-semibold text-lg mb-1">Registrar Desaparecido</h3>
        ...
        <div className="bg-white rounded-lg shadow p-4 mb-6">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg font-medium transition ${
                filter === 'person' ? 'bg-blue-600 text-white' : 'bg-blue-100 text-blue-800 hover:bg-blue-200'
              }`}
            >
              Pessoas
            </button>
            <button
              ...
              className={`px-4 py-2 rounded-lg font-medium transition ${
                filter === 'child' ? 'bg-pink-600 text-white' : 'bg-pink-100 text-pink-800 hover:bg-pink-200'
              }`}
            >
              Crianças
            </button>
            <button
              ...
              className={`px-4 py-2 rounded-lg font-medium transition ${
                filter === 'animal' ? 'bg-green-600 text-white' : 'bg-green-100 text-green-800 hover:bg-green-200'
              }`}
            >
              Animais
            </button>
        ...
        <div className="space-y-4">
          <h2 className="text-xl font-bold mb-4">Desaparecidos ({filteredMissing.length})</h2>
          {filteredMissing.length === 0 ? (
            ...
            <div className="grid md:grid-cols-2 gap-4">
              {filteredMissing.map(missing => {
                ...
                return (
                  <div key={missing.id} className="bg-white rounded-lg shadow hover:shadow-lg transition p-6">
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          {missing.age && (
                            <span className="text-sm text-gray-600">({missing.age} anos)</span>
                          )}
                        </div>
                        <span className={`inline-block px-3 py-1 rounded-full text-xs font-medium mb-3 ${typeColors[missing.type]}`}>
                          {typeLabels[missing.type]}
                        </span>
                        <p className="text-sm text-gray-700 mb-3">{missing.description}</p>
                        <div className="space-y-1 text-sm text-gray-600 mb-4">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {new Date(missing.lastSeenDate).toLocaleString('pt-BR')}
                          </div>
                        </div>
                        <div className="border-t pt-3">
                          <p className="text-xs text-gray-600 mb-1">Contato para informações:</p>
          ...
          <div className="bg-white rounded-lg max-w-lg w-full p-6 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <select
                  value={formData.type}
                  onChange={(e) => setFormData(prev => ({ ...prev, type: e.target.value as MissingType }))}
              ...
              <div>
                <label className="block text-sm font-medium mb-2">Local Onde Foi Visto(a) Pela Última Vez</label>
                <input
                  type="text"
